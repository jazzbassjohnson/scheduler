...
class ZipScheduler:
    ...
    def queue_order(self, order: Order) -> None:
        ...
        # sort the unfulfilled orders by priority and time
        self.unfulfilled_orders.sort(key=lambda x: (x.priority != 'Emergency', x.time))
    ...
    def launch_flights(self, current_time: int) -> List[Flight]:
        ...
        # The list of flights to be launched
        flights = []

        # While there are unfulfilled orders and there are Zips available...
        while self.unfulfilled_orders and len(flights) < self.num_zips:
            flight_orders = []
            total_distance = 0
            package_count = 0
            current_position = (0, 0)  # Nest position

            # Copy of the unfulfilled orders to iterate over
            orders_to_consider = self.unfulfilled_orders[:]

            # Iterate through the unfulfilled orders
            for order in orders_to_consider:
                # Check if we have reached the max packages per Zip
                if package_count >= self.max_packages_per_zip:
                    break

                hospital_position = (order.hospital.north_m, order.hospital.east_m)

                # Calculate distance to hospital (from current_position)
                distance_to_hospital = self.calculate_distance(current_position, hospital_position)
                # Calculate round trip distance (including return to nest)
                round_trip_distance = total_distance + distance_to_hospital + self.calculate_distance(hospital_position, (0, 0))

                # If the round trip distance is within the max cumulative range,
                # add the order to the flight_orders list
                if round_trip_distance <= self.zip_max_cumulative_range_m:
                    flight_orders.append(order)
                    self._unfulfilled_orders.remove(order)  # Remove the order from unfulfilled orders
                    total_distance += distance_to_hospital
                    package_count += 1
                    current_position = hospital_position

                    notify_hospital(order, distance_to_hospital, self.zip_speed_mps)

            if flight_orders:
                flights.append(Flight(current_time, flight_orders))

        return flights

    @staticmethod
    def calculate_distance(current_position, hospital_position):
        # returns the manhattan distance between two points
        return abs(hospital_position[0] - current_position[0]) + abs(hospital_position[1] - current_position[1])
